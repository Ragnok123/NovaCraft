...
class Physics {
    ...
    private boolean checkJump(Rectangle rect, int dir) {
        ...
        return (bl > 0 && Items.getBlock(bl).toJump() &&
                (rect.y + rect.height) - Items.getBlock(bl).getRect(blX / 16, blY / 16).y > 8);
    }
    ...
    private boolean checkColl(Rectangle rect) {
        ...
        for (int y = minY; y < maxY; y++) {
            for (int x = minX; x < maxX; x++) {
                ...
                if (bl > 0 && Items.getBlock(bl).hasCollision()) {
                    if (Intersector.overlaps(rect, Items.getBlock(bl).getRect(x, y))) {
                        return true;
                    }
                }
            }
    ...
    private void playerPhy(Player pl) {
        ...
        if (Items.isFluid(getBlock(pl.getRect()))) {
            if (NovaCraft.TOUCH && pl.mov.x != 0 && !pl.swim && !pl.flyMode) pl.swim = true;
            if (!pl.swim) {
        ...
        if (NovaCraft.TOUCH && checkJump(pl.getRect(), pl.getDir()) && !pl.flyMode && pl.canJump && pl.mov.x != 0) {
            pl.mov.add(0, -8);
            pl.canJump = false;
        }
    }
